...
function createBaseMsgSend(): MsgSend {
  return { fromAddress: "", toAddress: "", amount: undefined };
}
...
function createBaseMsgReceive(): MsgReceive {
  return { receiver: "", transactionId: 0 };
}
...
export class MsgClientImpl implements Msg {
  ...
  Send(request: MsgSend): Promise<MsgSendResponse> {
    ...
    const promise = this.rpc.request("moon.ibank.Msg", "Send", data);
    return promise.then((data) => MsgSendResponse.decode(new _m0.Reader(data)));
  }

  Receive(request: MsgReceive): Promise<MsgReceiveResponse> {
    const data = MsgReceive.encode(request).finish();
    const promise = this.rpc.request("moon.ibank.Msg", "Receive", data);
